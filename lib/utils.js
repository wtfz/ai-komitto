export function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === '--max-words' || arg === '-w') args.maxWords = argv[++i];
    else if (arg.startsWith('--max-words=')) args.maxWords = arg.split('=')[1];
...
const FORMAT_RULES = {
  ...
  full: [
    '- write a short title line (max ${maxWords} words) then a blank line then a body',
...
export function buildPrompt(diff, language = 'english', maxWords = 12, format = 'conventional', context = '') {
...
Options:
  ...
  -w, --max-words <n>     max words in commit message, default: 12
...
Environment:
  ...
  KOMITTO_MAX_WORDS       max words in commit message, default: 12
